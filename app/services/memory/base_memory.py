"""
Base memory architecture for RAG system
Implements OpenAI + ChromaDB integration for ai-agents-fork
"""
...
class BaseMemorySystem(ABC):
    """Base class for memory systems integrated with ai-agents-fork"""
    
    def __init__(self, name: str):
        self.name = name
        self.conversation_count = 0
        
        # Set up OpenAI client using ai-agents-fork config
        self.openai_client = openai.OpenAI(api_key=settings.OPENAI_API_KEY)
        
        # Set up ChromaDB client using ai-agents-fork config
    ...
    def get_embedding(self, text: str) -> List[float]:
        """Get OpenAI embedding for text using ai-agents-fork config"""
        try:
    ...
    def extract_facts(self, user_message: str, assistant_response: str) -> Dict[str, Any]:
        """Extract structured facts from conversation using ai-agents-fork LLM config"""
        try:
