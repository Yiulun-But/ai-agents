"""
Web Search Service for ai-agents-fork RAG system
...
class WebSearchService:
    """Main web search service for ai-agents-fork RAG system"""
    ...
    def __init__(self):
        ...
        print(f"[SEARCH_SERVICE] WebSearchService initialized with providers: {list(self.providers.keys())}")
        print(f"[SEARCH_SERVICE] Provider preference order: {self.provider_order}")
    ...
    async def search_async(self, query: str, max_results: int = 5, provider: str = None) -> List[Dict[str, Any]]:
        """Search asynchronously using specified provider or auto-fallback"""
        import asyncio
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self.search, query, max_results, provider)
    ...
    def _search_internal(self, query: str, max_results: int = 5, provider: str = None) -> List[SearchResult]:
        ...
        if provider and provider in self.providers:
            if self.providers[provider].is_available():
                try:
                    ...
                    if results:
                        print(f"[SEARCH_SERVICE] Search completed using {provider}: {len(results)} results")
                        return results
                except Exception as e:
                    print(f"[SEARCH_SERVICE] Search with {provider} failed: {e}")
            else:
                print(f"[SEARCH_SERVICE] Provider {provider} not available")
        ...
        for provider_name in self.provider_order:
            ...
            try:
                ...
                if results:
                    print(f"[SEARCH_SERVICE] Search completed using {provider_name}: {len(results)} results")
                    return results
            except Exception as e:
                print(f"[SEARCH_SERVICE] Search with {provider_name} failed: {e}, trying next provider")
                continue
        
        print("[SEARCH_SERVICE] All search providers failed")
        return []
    ...
    def test_providers(self) -> Dict[str, bool]:
        ...
        print("[SEARCH_SERVICE] Testing all providers...")
        
        for name, provider in self.providers.items():
            if not provider.is_available():
                results[name] = False
                print(f"[SEARCH_SERVICE] Provider {name}: Not available")
                continue
                
            try:
                search_results = provider.search(test_query, max_results=1)
                results[name] = len(search_results) > 0
                print(f"[SEARCH_SERVICE] Provider {name}: {'✓ Working' if results[name] else '✗ No results'}")
            except Exception as e:
                print(f"[SEARCH_SERVICE] Provider {name}: ✗ Failed ({e})")
                results[name] = False
    ...
    def set_preferred_provider(self, provider_name: str) -> bool:
        ...
        if provider_name in self.providers and self.providers[provider_name].is_available():
            ...
            print(f"[SEARCH_SERVICE] Preferred provider set to: {provider_name}")
            return True
        return False
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            test_results = self.test_providers()
            working_providers = [name for name, working in test_results.items() if working]
